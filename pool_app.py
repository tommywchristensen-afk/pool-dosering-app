...
def add_pool(name, vol):
    sheet.append_row([name, vol, name, "", "", "", ""])  # Pool Navn, Volumen, Adresse, Pumpetype, Returskyl, Nøglebokskode, HE telefonnummer
...
if pool_list:
    ...
    st.caption(f"Adresse: {info.get('Adresse', 'Ikke angivet')} | Nøglebokskode: {info.get('Nøglebokskode', 'Ikke angivet')} | HE telefonnummer: {info.get('HE telefonnummer', 'Ikke angivet')} | Pumpetype: {info.get('Pumpetype', 'Ikke angivet')} | Returskyl (5 min): {info.get('Returskyl (5 min)', 'Ikke angivet')}")
else:
